...
def history_to_transcript(history: list) -> str:
    """Format a list of messages [{ role, content }] into the same transcript string as chat_log_to_text."""
    lines = []
    for m in history:
...
请从以下 4 个维度打分，每个维度 0–10 分（0 最低，10 最高）：
...
# 评估维度顺序（与 EVALUATOR_SYSTEM_PROMPT 及前端一致）
EVAL_DIM_KEYS = (
    "emotional_intimacy",
...
# 初始维度默认值（总感觉值 S = 平均）
EVAL_DIM_DEFAULT = [5.0, 5.0, 5.0, 5.0]
...
# 角色回复中禁止出现的词汇（不在对话里提及这些词）
FORBIDDEN_REPLY_TERMS = "情感、占有欲、试探、信任、性吸引、身体亲密"
FORBIDDEN_REPLY_PREFIX = f"[你的回复中禁止出现以下词汇：{FORBIDDEN_REPLY_TERMS}。]\n\n"


# 单维每轮最大增加量（正方向）
MAX_DELTA_PER_DIM = 0.5

# 评估加权：最新一轮 70%，之前最多 4 轮合计 30%
EVAL_WEIGHT_LATEST = 0.7
EVAL_WEIGHT_PREV4 = 0.3


def update_feeling(dimensions: list, deltas: list) -> tuple:
    """
    基于阈值和缩放的增量更新：总 S 单轮增加通常不超过 0.5；
    若所有维度都正增强则允许最多 1.0（20% 随机放宽）；负 delta 不缩放。
    单维每轮最多增加 MAX_DELTA_PER_DIM（0.5）。
    dimensions / deltas 为 4 维列表，会被原地修改；返回 (dimensions, new_S)。
    """
    n = len(dimensions)
    if n == 0 or n != len(deltas):
        return dimensions, sum(dimensions) / n if n else 0.0
    deltas = list(deltas)
    all_positive = all(d > 0 for d in deltas)
    max_increase = 1.0 if (all_positive and random.random() < 0.2) else 0.5
    proposed_delta_S = sum(deltas) / n
    if proposed_delta_S > max_increase:
        positive_sum = sum(max(0, d) for d in deltas)
        if positive_sum > 0:
            scale = max_increase * n / positive_sum
            for i in range(n):
                if deltas[i] > 0:
                    deltas[i] *= scale
    # 单维每轮最多增加 MAX_DELTA_PER_DIM（负向不限制）
    for i in range(n):
        if deltas[i] > MAX_DELTA_PER_DIM:
            deltas[i] = MAX_DELTA_PER_DIM
    # 负向增量 x2，单维值由下方 max(0.0, ...) 保证不低于 0
    for i in range(n):
        if deltas[i] < 0:
            deltas[i] = deltas[i] * 2
    for i in range(n):
        dimensions[i] = max(0.0, min(10.0, dimensions[i] + deltas[i]))
    new_S = sum(dimensions) / n
    return dimensions, new_S


def run_evaluation(client: Client, log_path: str, character_name: str, transcript_override: str = None) -> dict:
    """
    Run the evaluation agent in a new session (no continuation). If transcript_override
    is provided, use it; otherwise use the last 5 rounds from log_path. Returns a dict
    with keys emotional_intimacy, possessiveness_jealousy, testing_trust_building,
    sexual_attraction_physical_intimacy (0-10 each).
    """
    transcript = transcript_override if transcript_override else chat_log_to_text(log_path, last_n_rounds=5)
...
<html lang="zh-CN">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .eval-delta-up { color: #16a34a; }
      .eval-delta-down { color: #dc2626; }
      .eval-delta-zero { color: var(--text-secondary); }
  ...
  <body>
    ...
    <script>
      ...
      function renderEvaluation(scores, deltas) {
        ...
        EVAL_DIM_ORDER.forEach(function(key) {
          ...
          var delta = (deltas && deltas[key] != null) ? deltas[key] : null;
          var deltaStr = '';
          if (delta !== null && delta !== undefined) {
            if (delta > 0) deltaStr = ' <span class="eval-delta eval-delta-up">+' + delta + '</span>';
            else if (delta < 0) deltaStr = ' <span class="eval-delta eval-delta-down">' + delta + '</span>';
            else deltaStr = ' <span class="eval-delta eval-delta-zero">0</span>';
          }
          ...
          html += '<div class="eval-dim-score">' + val + ' / 10' + deltaStr + '</div>';
          html += '</div>';
        });
      ...
      function fetchEvaluation() {
        ...
        fetch('/evaluate', {
          ...
          .then(function(data) {
            if (data.scores) renderEvaluation(data.scores, data.deltas);
...
def evaluate_session():
    """
    Run the evaluation agent (separate session), then apply incremental update with scaling:
    - Raw scores from LLM → deltas vs current dimensions; positive deltas scaled so S increase ≤ 0.5 (or 1.0 if all positive + 20%).
    - Persist updated dimensions in session state; return scores dict for UI.
    """
    data = request.get_json(silent=True) or {}
    ...
    try:
        # 加权评估：最新一轮 70%，之前最多 4 轮合计 30%
        history = read_history_jsonl(log_path)
        num_rounds = len(history) // 2  # 1 round = 1 user + 1 assistant
        if num_rounds < 1:
            raw_scores = run_evaluation(client, log_path, character_name)
        else:
            rounds = [history[2 * i : 2 * i + 2] for i in range(num_rounds)]
            transcript_latest = history_to_transcript(rounds[-1])
            raw_latest = run_evaluation(client, log_path, character_name, transcript_override=transcript_latest)
            if num_rounds >= 2:
                prev4_rounds = rounds[-5:-1]  # 最多 4 轮（最后一轮之前）
                transcript_prev4 = history_to_transcript([m for r in prev4_rounds for m in r])
                raw_prev4 = run_evaluation(client, log_path, character_name, transcript_override=transcript_prev4)
                raw_scores = {
                    k: round(EVAL_WEIGHT_LATEST * raw_latest.get(k, 5.0) + EVAL_WEIGHT_PREV4 * raw_prev4.get(k, 5.0), 1)
                    for k in EVAL_DIM_KEYS
                }
            else:
                raw_scores = raw_latest
        if not raw_scores:
            return jsonify({"ok": True, "session_id": session_id, "scores": None}), 200
        # 当前维度：从 state 读取，若无则用本轮 LLM 原始分作为初始
        stored = state.get("evaluation_dimensions")
        if stored and len(stored) == len(EVAL_DIM_KEYS):
            dimensions = [float(x) for x in stored]
            old_dimensions = list(dimensions)
            # 本轮的“建议分”作为目标，delta = 建议 - 当前
            deltas = [float(raw_scores.get(k, dimensions[i])) - dimensions[i] for i, k in enumerate(EVAL_DIM_KEYS)]
            dimensions, new_S = update_feeling(dimensions, deltas)
            applied_deltas = [round(dimensions[i] - old_dimensions[i], 1) for i in range(len(EVAL_DIM_KEYS))]
        else:
            dimensions = [float(raw_scores.get(k, 5.0)) for k in EVAL_DIM_KEYS]
            dimensions = [max(0.0, min(10.0, d)) for d in dimensions]
            new_S = sum(dimensions) / len(dimensions)
            applied_deltas = [round(dimensions[i] - 5.0, 1) for i in range(len(EVAL_DIM_KEYS))]
        state["evaluation_dimensions"] = dimensions
        state["evaluation_S"] = round(new_S, 2)
        state["updated_at"] = time.time()
        write_json(state_path, state)
        scores = {k: round(dimensions[i], 1) for i, k in enumerate(EVAL_DIM_KEYS)}
        deltas_dict = {k: applied_deltas[i] for i, k in enumerate(EVAL_DIM_KEYS)}
        return jsonify({"ok": True, "session_id": session_id, "scores": scores, "deltas": deltas_dict})
    except Exception as e:
...
def chat_endpoint():
    ...
    # Build message to model: optional evaluation context + forbidden-reply terms + optional SFW prefix + user input
    message_to_model = FORBIDDEN_REPLY_PREFIX + user_input
    state_for_chat = read_json(STATE_PATH) or {}
    if isinstance(state_for_chat, dict):
        dims = state_for_chat.get("evaluation_dimensions")
        if dims and len(dims) == len(EVAL_DIM_KEYS):
            # 情感亲密度 < 7 时强制关闭 NSFW，即使前端开关为 ON
            if float(dims[0]) < 7:
                nsfw_allowed = False
            parts = [f"{EVAL_DIM_LABELS_ZH[i]}={float(dims[i]):.1f}" for i in range(len(EVAL_DIM_KEYS))]
            s_val = state_for_chat.get("evaluation_S")
            if s_val is None:
                s_val = sum(float(x) for x in dims) / len(dims)
            eval_ctx = "[当前角色对用户的感觉数值：" + "，".join(parts) + f"；总感觉值 S={float(s_val):.1f}。请根据这些数值调整回复风格，使对话内容与感觉数值保持一致。]\n\n"
            message_to_model = eval_ctx + message_to_model
    if not nsfw_allowed:
        message_to_model = "[Keep your reply SFW; avoid explicit sexual or adult content.] " + message_to_model

    # Append user message (with evaluation context and/or SFW instruction; log stores original input only)
    chat.append(user(message_to_model))
    append_jsonl(
        CHAT_LOG_PATH,
        {
            "type": "message",
