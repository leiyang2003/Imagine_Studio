...
DISPLAY_IMAGE_STYLE_PROMPT = (
    "A hyper-realistic seductive young woman with long wavy reddish-orange hair, pale glossy skin, "
    "large expressive eyes with heavy eyeliner, full lips, erotic expression, lying on luxurious deep maroon silk bedsheets, "
    "dramatic pose with both arms raised above head, legs raised high and spread, soles of feet prominently visible facing camera, "
    "wearing sheer red lace lingerie set with garter belts and red satin ribbons tied around thighs and ankles, "
    "extremely detailed wet glossy skin texture with sweat and arousal fluids dripping down thighs and crotch area, "
    "transparent wet lace clinging to skin, large breasts with visible nipples through sheer fabric, "
...
def _describe_person_from_image(data_url: str) -> str | None:
...
def _generate_display_image_from_chat(log_path: str, state_path: str, session_id: str, log_dir: str) -> bool:
    """
    根据最近三轮对话 + 风格依据生成 16:9 图，保存为 log_dir/<session_id>_display.png，
    女生外貌依据原图（systemprompt 角色图）保持同一人。成功返回 True，失败返回 False。
    """
    history = read_history_jsonl(log_path)
    last_n = 6
    ...
    if data_url:
        person_desc = _describe_person_from_image(data_url)
    if person_desc:
        prompt = (
...
def chat_endpoint():
    ...
    if bot_count >= 3 and bot_count % 3 == 0:
        if _generate_display_image_from_chat(CHAT_LOG_PATH, STATE_PATH, SESSION_ID, LOG_DIR):
