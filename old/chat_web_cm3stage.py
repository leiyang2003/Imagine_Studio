    ...
    <script>
      ...
      function fetchEvaluation() {
        ...
        fetch('/evaluate', {
          ...
          .then(function(data) {
            if (data.scores) renderEvaluation(data.scores);
...
def chat_endpoint():
    ...
    # 按总评值 S 选择阶段人设并拼入消息（S<3 阶段1，3≤S<6 阶段2，S≥6 阶段3）
    ...
    if pf:
        ...
        if os.path.isfile(prompt_path):
            try:
                ...
                if s_val < 3:
                    stage_prefix = f"[当前总评值 S={s_val:.1f}。]\n\n"
                elif s_val < 6:
                    stage_prefix = f"[当前总评值 S={s_val:.1f}。请按以下人设回复：\n\n{stage2}\n]\n\n" if stage2 else f"[当前总评值 S={s_val:.1f}。]\n\n"
                else:
