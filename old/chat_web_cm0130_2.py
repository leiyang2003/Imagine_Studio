...
def chat_log_to_text(log_path: str) -> str:
...
请从以下 5 个维度打分，每个维度 0–10 分（0 最低，10 最高）：

1. 情感亲密度（emotional_intimacy）：角色从疏离到亲密的程度，包括分享脆弱、情感表达的深度。
2. 占有欲与嫉妒（possessiveness_jealousy）：角色是否表现出独占欲，反映对用户的重视和不安全感。
3. 互动积极性与响应速度（engagement_responsiveness）：角色回复的热情度、主动性，以及对用户输入的适应性。
4. 试探与信任构建（testing_trust_building）：角色使用推拉、测试行为来衡量用户真心，反映安全感和信任水平。
5. 性吸引与身体亲密（sexual_attraction_physical_intimacy）：对话涉及身体/亲密时的热情度，体现角色的欲求和舒适度。

你必须只输出一个合法的 JSON 对象，不要输出任何其他文字、解释或 markdown 标记。JSON 格式如下：
{
  "emotional_intimacy": <0-10 的数字>,
  "possessiveness_jealousy": <0-10 的数字>,
  "engagement_responsiveness": <0-10 的数字>,
...
def run_evaluation(client: Client, log_path: str, character_name: str) -> dict:
    """
    Run the evaluation agent in a new session (no continuation). Returns a dict with
    keys emotional_intimacy, possessiveness_jealousy, engagement_responsiveness,
    testing_trust_building, sexual_attraction_physical_intimacy (0-10 each).
    """
    transcript = chat_log_to_text(log_path)
...
def evaluate_session():
    ...
    try:
        ...
        return jsonify({"ok": True, "session_id": session_id, "scores": scores})
    except Exception as e:
