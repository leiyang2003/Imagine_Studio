...
async def _tts_stream_voice_api_into_queue(text: str, api_key: str, out_queue: "queue.Queue[str | None]") -> None:
    """
    与 _tts_via_voice_api 相同流程，但每收到一个 output_audio.delta 就把 base64 放入 out_queue，
    结束时 put(None)。供流式端点 /tts-stream 使用，实现「边收边播」。
    """
    import websockets
...
# 并行优化：chat 返回后立即在后台启动 TTS 与评估，前端请求时命中缓存则立即返回
TTS_CACHE = {}  # (session_id, text_hash) -> (wav_bytes, timestamp)
TTS_CACHE_LOCK = threading.Lock()
TTS_CACHE_TTL = 300  # 秒
PENDING_EVALUATION = {}  # (session_id, round_count) -> {"event": Event(), "result": payload}
PENDING_EVALUATION_LOCK = threading.Lock()


def _tts_cache_key(session_id: str, text: str) -> tuple:
    h = hashlib.sha256(text.encode("utf-8")).hexdigest()[:32]
    return (session_id, h)


def _run_evaluate_for_session(session_id: str) -> dict:
    """评估逻辑：读 log/state，若 round_count % 5 == 0 则调 LLM 并写回 state，返回 payload。"""
    log_path = os.path.join(LOG_DIR, f"{session_id}.jsonl")
    state_path = os.path.join(LOG_DIR, f"{session_id}.state.json")
    if not os.path.isfile(log_path):
        return {"ok": False, "error": "session not found", "scores": None}
    state = read_json(state_path) or {}
    if not isinstance(state, dict):
        state = {}
    ...
    try:
        ...
        if round_count < 5 or round_count % 5 != 0:
            ...
            if stored and len(stored) == len(EVAL_DIM_KEYS):
                scores_return = {k: round(float(stored[i]), 1) for i, k in enumerate(EVAL_DIM_KEYS)}
            else:
                scores_return = None
            return {
                "ok": True,
                "session_id": session_id,
                "scores": scores_return,
        ...
        for _ in range(max_retries):
            scores = run_evaluation(client, log_path, character_name)
            if scores:
                break
        if scores:
            ...
            state["updated_at"] = time.time()
            write_json(state_path, state)
            scores_return = {k: round(float(state["evaluation_dimensions"][i]), 1) for i, k in enumerate(EVAL_DIM_KEYS)}
            return {
                "ok": True,
                "session_id": session_id,
                "scores": scores_return,
        ...
        return {"ok": False, "error": "评估返回不完整（缺少维度），已重试 3 次，请稍后再试", "scores": None}
    except Exception as e:
        return {"ok": False, "error": str(e), "scores": None}
  ...
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      #system-prompt-content.system-prompt-scroll {
        width: 100%;
        max-width: 480px;
        max-height: 200px;
        overflow-y: auto;
  ...
  <body>
    ...
    <div id="chat-screen">
      ...
      <div id="system-prompt-wrap">
        <h3>当前 System Prompt</h3>
        <div id="system-prompt-content" class="system-prompt-scroll">加载中…</div>
      </div>
    </div>
    <script>
      ...
      function fetchEvaluation() {
        ...
        fetch('/evaluate', {
          ...
          .then(function(data) {
            if (data.scores) {
              var stageData = (data.previous_stage_ps != null || data.new_stage_ps != null) ? {
      ...
      function renderPersonList(files) {
        ...
        if (!files || files.length === 0) {
          el.innerHTML = '<div class="picker-empty">暂无可选角色，请在 systemprompt 目录下添加 .txt 文件</div>';
          return;
        }
      ...
      function loadPromptFiles() {
        var el = document.getElementById('person-list');
        if (!el) return;
        el.innerHTML = '<div class="picker-empty">加载中…</div>';
        var timeoutId;
        var abort = new AbortController();
        timeoutId = setTimeout(function() { abort.abort(); }, 12000);
        fetch('/prompt-files', { signal: abort.signal })
          .then(function(res) {
          ...
          .then(function(text) {
            var data;
            try { data = JSON.parse(text); } catch (e) { throw new Error('响应格式错误'); }
            var list = (data && data.prompt_files) ? data.prompt_files : [];
            renderPersonList(Array.isArray(list) ? list : []);
          })
          .catch(function(err) {
            clearTimeout(timeoutId);
            var msg = err.name === 'AbortError' ? '加载超时，请检查网络或刷新重试' : ('加载失败: ' + (err.message || '请刷新重试'));
            var listEl = document.getElementById('person-list');
            if (listEl) listEl.innerHTML = '<div class="picker-empty">' + msg + ' <button type="button" class="retry-btn" style="margin-left:8px;padding:4px 8px;">重试</button></div>';
            var retryBtn = listEl && listEl.querySelector('.retry-btn');
            if (retryBtn) retryBtn.onclick = loadPromptFiles;
          });
      }
      if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', loadPromptFiles);
      } else {
        loadPromptFiles();
      }
      ...
      function playTts(text) {
        ...
        return fetch('/tts-stream', {
          ...
          if (!reader || myGen !== ttsGeneration) return;
          var ctx = new (window.AudioContext || window.webkitAudioContext)();
          var nextStart = ctx.currentTime;
          var buf = '';
          function decodeBase64ToPcm(base64Str) {
            var binary = atob(base64Str);
            var bytes = new Uint8Array(binary.length);
            for (var i = 0; i < binary.length; i++) bytes[i] = binary.charCodeAt(i);
            return bytes.buffer;
          }
          function playPcmChunk(pcmArrayBuffer) {
            if (myGen !== ttsGeneration) return;
            var numSamples = pcmArrayBuffer.byteLength / 2;
            if (numSamples === 0) return;
            var buffer = ctx.createBuffer(1, numSamples, TTS_SAMPLE_RATE);
            var int16 = new Int16Array(pcmArrayBuffer);
            var ch = buffer.getChannelData(0);
            for (var i = 0; i < numSamples; i++) ch[i] = int16[i] / 32768;
            var src = ctx.createBufferSource();
            src.buffer = buffer;
            src.connect(ctx.destination);
            var startAt = nextStart;
            nextStart += buffer.duration;
            if (ctx.state === 'suspended') {
              ctx.resume().then(function () { src.start(startAt); });
            } else {
              src.start(startAt);
            }
          }
          function pump() {
            return reader.read().then(function (r) {
              if (r.done || myGen !== ttsGeneration) {
                if (myGen === ttsGeneration) status.textContent = '准备就绪。';
                return;
              }
              ...
              parts.forEach(function (block) {
                var idx = block.indexOf('data:');
                if (idx === -1) return;
                var payload = block.slice(idx + 5).trim();
                if (!payload) return;
                try {
                  var pcm = decodeBase64ToPcm(payload);
                  if (pcm.byteLength >= 2) {
...
def get_prompt_files():
    ...
    try:
        ...
        return jsonify({"prompt_files": files or []})
    except Exception as e:
        append_error_log("prompt-files", str(e))
...
def evaluate_endpoint():
    """
    Run the evaluation agent. 若 chat 已触发后台评估，则等待/返回缓存结果，缩短延迟。
    """
    data = request.get_json(silent=True) or {}
    ...
    history = read_history_jsonl(log_path)
    round_count = len(history) // 2
    key = (session_id, round_count)
    with PENDING_EVALUATION_LOCK:
        pend = PENDING_EVALUATION.get(key)
    if pend is not None:
        pend["event"].wait(timeout=90)
        with PENDING_EVALUATION_LOCK:
            result = PENDING_EVALUATION.get(key, {}).get("result")
            if key in PENDING_EVALUATION:
                del PENDING_EVALUATION[key]
        if result is not None:
            return jsonify(result)
    payload = _run_evaluate_for_session(session_id)
...
def chat_endpoint():
    ...
    # Append user message (with SFW instruction if applicable)
    ...
    session_id = SESSION_ID
    history_len = len(read_history_jsonl(CHAT_LOG_PATH))
    round_count = history_len // 2

    def _bg_tts():
        try:
            wav_bytes = asyncio.run(_tts_via_voice_api(reply_text, api_key))
            if wav_bytes:
                key = _tts_cache_key(session_id, reply_text)
                with TTS_CACHE_LOCK:
                    TTS_CACHE[key] = (wav_bytes, time.time())
        except Exception:
            pass

    def _bg_evaluate():
        try:
            result = _run_evaluate_for_session(session_id)
            with PENDING_EVALUATION_LOCK:
                if (session_id, round_count) in PENDING_EVALUATION:
                    PENDING_EVALUATION[(session_id, round_count)]["result"] = result
                    PENDING_EVALUATION[(session_id, round_count)]["event"].set()
        except Exception:
            with PENDING_EVALUATION_LOCK:
                if (session_id, round_count) in PENDING_EVALUATION:
                    PENDING_EVALUATION[(session_id, round_count)]["result"] = {"ok": False, "error": "background eval failed", "scores": None}
                    PENDING_EVALUATION[(session_id, round_count)]["event"].set()

    threading.Thread(target=_bg_tts, daemon=True).start()
    if round_count >= 5 and round_count % 5 == 0:
        with PENDING_EVALUATION_LOCK:
            PENDING_EVALUATION[(session_id, round_count)] = {"event": threading.Event(), "result": None}
...
def tts_endpoint():
    ...
    若 chat 已触发后台 TTS，命中缓存则立即返回，缩短延迟。
    """
    data = request.get_json(silent=True) or {}
    ...
    with TTS_CACHE_LOCK:
        if key in TTS_CACHE:
            ...
            del TTS_CACHE[key]
    try:
        wav_bytes = asyncio.run(_tts_via_voice_api(text, api_key))
    except Exception as e:
...
def _tts_stream_generator(text: str):
    """SSE 流：每收到 Voice API 一个 audio delta 就 yield 给前端，实现边收边播。"""
    q = queue.Queue()
    def run():
        try:
            asyncio.run(_tts_stream_voice_api_into_queue(text, api_key, q))
        except Exception:
            q.put(None)
...
def tts_stream_endpoint():
    """
    流式 TTS：每收到 Voice API 的音频片段就推给前端（SSE），前端边收边播，降低首包延迟。
    Body: { "text": "..." }。响应为 text/event-stream，每行 data: <base64 的 PCM delta>。
    """
    data = request.get_json(silent=True) or {}
    text = (data.get("text") or "").strip()
    if not text:
        return jsonify({"error": "missing or empty text"}), 400
    return Response(
        stream_with_context(_tts_stream_generator(text)),
